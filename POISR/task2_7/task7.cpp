...
struct Params {
    ...
    int winsize; // размер окна на этапе предварительного сглаживани€
    int iterations; // число итераций на каждом уровне пирамиды
    int polyN; // размер окрестности дл€ аппроксимации полиномом
    double polySigma; // стандартное отклонение €дра √аусса дл€ вычислени€
...
static void apply_transform(string movement_video_path, cv::Mat& transform, string stats_txt_path, Params& params) {
    ...
    {   
        ...
        //// ѕолутоновое изображение дл€ отладочного пол€
        //cv::Mat debugImage = cv::Mat::zeros(frame.size(), CV_8UC1);

        //// ¬ычисление масштаба дл€ нормализации длины векторов
        //cartToPolar(flow_parts[0], flow_parts[1], magnitude, angle);
        //double maxMagnitude;
        //minMaxLoc(magnitude, nullptr, &maxMagnitude);
        //
        //double scale = 20.0 / maxMagnitude; // Ќормализаци€ длины векторов (шаг 20 пикселов)
        //int step = 20;
